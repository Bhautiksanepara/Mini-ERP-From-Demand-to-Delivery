import { Boxes, Menu, Plus, RefreshCw, XCircle } from "lucide-react";
  ...
  if (config.statusFilters?.[label]?.movement_direction) {
    return rows.filter(
      (row) =>
        ...
        config.statusFilters[label].movement_direction,
    ).length;
  }

  if (config.statusFilters?.[label]?.movement_type) {
    return rows.filter(
      (row) => row.movement_type === config.statusFilters[label].movement_type,
    ).length;
  }
    ...
    <main className="grid gap-5 p-6 max-md:p-4">
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          />
          {/* Drawer Body */}
          <div className="relative w-full max-w-4xl bg-white shadow-2xl flex flex-col h-full z-50 animate-slide-in overflow-hidden">
            {drawerLoading ? (
              <div className="flex flex-col h-full items-center justify-center gap-3 text-slate-500 bg-slate-50">
