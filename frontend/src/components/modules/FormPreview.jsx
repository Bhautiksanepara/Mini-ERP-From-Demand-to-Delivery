  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 text-left">
      ...
      <main ref={bodyRef} className="flex-1 overflow-y-auto p-6 space-y-6">
        {message && (
          <div className={`rounded-xl border px-4 py-3 text-sm font-bold shadow-xs flex items-center gap-2 ${
            isError 
...
function OrderEntryForm({ config, moduleKey, onCreated, selectedRecord: initialSelectedRecord, onClose }) {
  ...
  const isReadOnly = selectedRecord && selectedRecord.status !== 'Draft';
...
function ManufacturingOrderForm({ config, onCreated, selectedRecord: initialSelectedRecord, onClose }) {
  const initialForm = {
    finished_product_id: '',
    quantity: '1',
    unit: 'Units',
  ...
  const isReadOnly = selectedRecord && !['Draft', 'Confirmed', 'In Progress'].includes(selectedRecord.status);
    ...
    details.push({ 
      ...
      value: (
        <span className={`rounded-full px-2.5 py-0.5 text-xs font-extrabold ${
          selectedRecord.action === 'Created' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
...
export function FormPreview({ config, moduleKey, onCreated, selectedRecord, onClose }) {
  if (moduleKey === 'sales' || moduleKey === 'purchase') {
    return <OrderEntryForm config={config} moduleKey={moduleKey} onCreated={onCreated} selectedRecord={selectedRecord} onClose={onClose} />;
  }

  if (moduleKey === 'manufacturing') {
    return <ManufacturingOrderForm config={config} onCreated={onCreated} selectedRecord={selectedRecord} onClose={onClose} />;
  }
